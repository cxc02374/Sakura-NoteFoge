...
- プレビュー内リンク遷移（外部ブラウザに飛ばず同一ペインで表示）
- プレビュー右クリックメニュー（戻る / 進む / 再読み込み）
- プレビュー本文の日本語変換（右クリック: `日本語に変換（Google翻訳）`）
- 見出しアウトライン表示（左ペイン、クリックでジャンプ）
- 検索 (Ctrl+F) / 置換 (Ctrl+H)
- ライト/ダークテーマ切替（`表示 > テーマ`）
- 画像ファイルのドラッグ＆ドロップで `![]()` 自動挿入
- 現在ドキュメントの再読み込み（`F5` / `ファイル > 再読み込み`）
- 履歴スナップショット保存（保存時）
- 自動保存（編集中の復旧用）
...
.\scripts\build_windows_installer.ps1 -AppVersion "1.1.0"
```
...
```powershell
# 例: v1.1.0 をリリース
git tag -a v1.1.0 -m "v1.1.0: release note"
...
- **保存**: `Ctrl+S`
- **再読み込み**: `F5`（現在タブのファイルをディスクから再読込）
- **PDF出力**: `ファイル > PDFとして出力`
- 画像はエディタにドラッグ＆ドロップしてください。

### プレビューのブラウズ/翻訳
- リンククリックで、プレビュー内をそのまま遷移します（外部ブラウザへはジャンプしません）。
- プレビュー上で右クリックすると `戻る / 進む / 再読み込み / 日本語に変換（Google翻訳）` を使用できます。
- `日本語に変換` は、現在表示中の本文を日本語へ変換し、同じプレビュー内に結果を表示します。
